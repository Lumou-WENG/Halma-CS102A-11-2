...
public class ChessBoardComponent extends JComponent {
    private static final Color BOARD_COLOR_1 = new Color(253, 255, 220);
        ...
        for (int row = 0; row < dimension; row++) {
            for (int col = 0; col < dimension; col++) {
                gridComponents[row][col] = new SquareComponent(gridSize,
                        (row + col) % 2 == 0 ? BOARD_COLOR_1 : BOARD_COLOR_2);//颜色交替
                gridComponents[row][col].setLocation(row * gridSize, col * gridSize);
                add(gridComponents[row][col]);//可视化
            }
        }
    }
    ...
    public void setChessAtGrid(ChessBoardLocation location, Color color) {
        removeChessAtGrid(location);//移开当前位置棋子
        getGridAt(location).add(new ChessComponent(color));//把棋子加到显示队列中
    }
